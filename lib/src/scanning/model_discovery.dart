...
/// A directory that looks like it holds API models.
class ModelsCandidate {
  /// Relative to the project root, as the config stores it.
  final String relative;

  /// How many classes there declare `fromJson`/`toJson`.
  final int classCount;

  const ModelsCandidate(this.relative, this.classCount);
}

/// Directories under [searchRoot] that declare model classes, richest first.
///
/// Offered by `init` so the directory can be picked rather than typed, which
/// is the step people get wrong. Only [_modelClassesIn] decides — a file that
/// merely *calls* `Foo.fromJson` is a consumer, not a model, so `lib/ui`
/// never shows up here.
///
/// The substring pass before parsing is what keeps this quick on a large
/// project: reading a few thousand files is IO-bound, parsing them all is
/// not, and only a small fraction can possibly qualify.
Future<List<ModelsCandidate>> findModelsCandidates({
  required String projectRoot,
  String searchRoot = 'lib',
}) async {
  final root = Directory(p.join(projectRoot, searchRoot));
  if (!root.existsSync()) {
    return const [];
  }
  ...
  for (final file in root.listSync(recursive: true).whereType<File>()) {
    ...
    try {
      ...
      continue; // Unreadable or not text; nothing to learn from it.
    }
    ...
    final int declared;
    try {
      declared = _modelClassesIn(parseString(
        content: content,
        path: file.path,
      ).unit).length;
    } catch (_) {
      continue; // A file that will not parse cannot vouch for a directory.
    }
    ...
    final directory = p.relative(file.parent.path, from: projectRoot);
    counts[directory] = (counts[directory] ?? 0) + declared;
  }

  final candidates = [
    for (final entry in counts.entries)
      ModelsCandidate(entry.key, entry.value),
  ]..sort((a, b) {
      final byCount = b.classCount.compareTo(a.classCount);
      return byCount != 0 ? byCount : a.relative.compareTo(b.relative);
    });
